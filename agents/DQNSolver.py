...
class DQNAgent:
    ...
    def __init__(self, state_space, action_space, max_memory_size, batch_size, gamma, lr,
                 dropout, exploration_max, exploration_min, exploration_decay, double_dqn, pretrained, path):
        ...
        if self.pretrained:
            self.STATE_MEM = torch.load(os.path.join(path,"STATE_MEM.pt"))
            self.ACTION_MEM = torch.load(os.path.join(path,"ACTION_MEM.pt"))
            self.REWARD_MEM = torch.load(os.path.join(path,"REWARD_MEM.pt"))
            self.STATE2_MEM = torch.load(os.path.join(path,"STATE2_MEM.pt"))
            self.DONE_MEM = torch.load(os.path.join(path,"DONE_MEM.pt"))
            with open(os.path.join(path,"ending_position.pkl"), 'rb') as f:
                self.ending_position = pickle.load(f)
            with open(os.path.join(path,"num_in_queue.pkl"), 'rb') as f:
